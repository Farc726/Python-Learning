# 要想有文件的持久化存储
#json文件与你的python结合非常合适
# 过程中因为你一开始写的储存信息的数据容器就是字典 所以在添加查询删除函数中只需要在字典更新完成后进行写入操作W即可 此时用到dump（字典，要写入的文件）
# 特殊 每次执行任务系统开始的时候 要打开文件去初始化字典 那这时候就用到了异常处理的情况 分为第一次打开与后续打开时
# 过程中遇到的其他细节
import json
